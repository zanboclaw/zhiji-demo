    ...
    <div className="space-y-5">
      <div className="rounded-[2rem] border border-white/8 bg-[linear-gradient(180deg,rgba(14,19,27,0.94),rgba(10,14,20,0.92))] p-5 shadow-[0_18px_46px_rgba(2,6,23,0.28)] lg:p-6">
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-3 text-3xl font-semibold tracking-tight text-white lg:text-[2.7rem]">{copy.title}</h1>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-gray-500">
              {copy.highlights.map((item, index) => (
                <span key={item} className="inline-flex items-center gap-2">
                  <span className={`h-1.5 w-1.5 rounded-full ${index < 2 ? 'bg-status-success' : 'bg-sky-400'}`} />
                  {item}
                </span>
              ))}
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:min-w-[520px] xl:grid-cols-4">
            <OverviewCard title={copy.metrics.online} value={onlineCount} unit="fleet" tone="success" bars={[28, 34, 42, 40, 44, 50, 58]} />
            <OverviewCard title={copy.metrics.warning} value={warningCount} unit="warning" tone="warning" bars={[42, 38, 34, 30, 26, 22, 18]} />
            <OverviewCard title={copy.metrics.offline} value={offlineCount} unit="offline" tone="danger" bars={[18, 20, 16, 14, 13, 12, 10]} />
            <OverviewCard title={copy.metrics.latency} value="42" unit="ms" tone="primary" bars={[22, 28, 26, 30, 34, 32, 36]} />
          </div>
        </div>
        ...
        <div className="mt-5 rounded-[1.55rem] border border-white/8 bg-black/15 px-4 py-4">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-[11px] uppercase tracking-[0.2em] text-gray-500">{copy.currentTarget}</div>
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  ...
                  <span className={`rounded-full border px-2.5 py-1 text-[11px] ${getRobotStatusBadgeClass(selectedRobotMeta.status)}`}>
                    {getRobotStatusLabel(selectedRobotMeta.status, locale)}
                  </span>
                </div>
            ...
            <div className="flex flex-wrap gap-2 text-xs text-gray-500">
              <span className="rounded-full border border-white/8 bg-white/[0.03] px-3 py-1.5">{copy.onlineChip(onlineCount)}</span>
              <span className="rounded-full border border-status-warning/18 bg-status-warning/10 px-3 py-1.5 text-status-warning">{copy.warningChip(warningCount)}</span>
              <span className="rounded-full border border-status-danger/18 bg-status-danger/10 px-3 py-1.5 text-status-danger">{copy.offlineChip(offlineCount)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <Card className="rounded-[1.85rem] border-white/8 bg-[rgba(13,18,25,0.9)] p-5 shadow-[0_16px_40px_rgba(2,6,23,0.22)]">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="text-[11px] uppercase tracking-[0.2em] text-gray-500">{copy.rosterEyebrow}</div>
            <div className="mt-2 text-lg font-semibold text-white">{copy.rosterTitle}</div>
          </div>
          <div className="text-sm text-gray-500">{copy.rosterDescription(onlineCount)}</div>
        </div>
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={`rounded-full border px-2.5 py-1 text-[11px] ${getRobotStatusBadgeClass(robot.status)}`}>
                        {getRobotStatusLabel(robot.status, locale)}
                      </span>
                    </div>
